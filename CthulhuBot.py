...
def processServerClick(serverId):
	...
	second_temp_array = []
	for server in guilds:
		print(str(serverId) + "---" + str(server.id))
		if int(serverId) == server.id:
			print(server.categories[-1])
				...
				for channel in category.channels:
					if bot.user in channel.members:
						if len(temp_array) > 0:
							...
							temp_array.append(second_temp_array)
							second_temp_array = []
				if len(temp_array) > 0 and channel == category.channels[-1]:
				...
				if "<@" in message.content:
					while message.content.find("<@") != -1:
						...
						if message.content[start+2] == "!":
							...
							for member in members:
								user = message.content[start+2:end].replace(">", "")
...
async def send_message(): # function to keep checking for a new message to send; resets message once sent; non-blocking
	...
	while True:
		...
		if message_to_send != "" and message_to_send != None and message_to_send != message_to_check:
			...
			if message_to_send == "!test":
				test_embed = discord.Embed(title='Test', description="Test", color=0x9b59b6)
				# test_embed.add_field(name='Message', value="Message:", inline=False)
				# test_embed.add_field(name='Content', value=message_to_send, inline=False)
				new_embed = discord.Embed(title='Test', description="Hi", color=0x9b59b6)
				# new_embed.add_field(name='Message', value="Message:", inline=False)
				# new_embed.add_field(name='Content', value="Hi", inline=False)
				msg = await channel.send(embed = test_embed)
				message_to_send = ""
				message_to_check = ""
				await msg.edit(embed = new_embed)
			elif message_to_send.startswith("!"):
				content = message_to_send[1:]
				new_embed = discord.Embed(title='Test', description=content, color=0x9b59b6)
				# new_embed.add_field(name='Message', value="Message:", inline=False)
				# new_embed.add_field(name='Content', value=content, inline=False)
				message_to_send = ""
				message_to_check = ""
				await msg.edit(embed = new_embed)
			else:
...
# def runClient():
 # bot's token; will be omitted from GitHub pushes
...
print(web_path)
eel.init(web_path)		# declares the directory for frontend files; for Eel
...
def runEelStartup():
	...
	electron_path = resource_path('node_modules/electron/dist/electron')
	start_path = resource_path('.')
	print(electron_path)
	print(start_path)
	eel.start('loading.html', mode='custom', cmdline_args=[electron_path, start_path])
	eel.start('index.html', mode='custom', cmdline_args=[electron_path, start_path])

threading.Thread(target=runEelStartup).start()
bot.run("NzUwNzU3MDAwMjk5OTM3OTM0.X0_K8Q.9seq5C3Jm5pVmnha_4H74Tqan4E")	